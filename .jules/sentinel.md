...
## 2026-04-19 - Command Injection Risk in Model Download Script
**Vulnerability:** The script `download_models.js` used `child_process.exec` to run a `git clone` command constructed via string interpolation. While the URLs and paths were hardcoded in this specific instance, this pattern is highly susceptible to command injection if any of the inputs become dynamic or user-controlled.
**Learning:** Using `exec` with string concatenation or interpolation is dangerous because the entire string is passed to a shell, allowing attackers to inject arbitrary commands using characters like `;`, `|`, or `&&`.
**Prevention:** Always use `child_process.execFile` (or `spawn`) with an array of arguments when executing external commands. This approach passes the arguments directly to the executable without involving a shell, neutralizing command injection vulnerabilities.
