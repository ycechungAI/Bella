...
## 2026-07-25 - Object-Based XSS Bypass in Formatting Logic
**Vulnerability:** The `escapeHtml` and `formatMessage` functions in `chatInterface.js` relied on `unsafe.replace(...)` directly without coercing the input to a string first. An attacker could bypass XSS sanitization by passing an object with a custom `replace` method that returned an unescaped malicious string. Additionally, the falsy check `if (!unsafe)` would incorrectly suppress valid inputs like the number `0`.
**Learning:** Checking truthiness (`if (!input)`) and calling string methods directly on inputs is unsafe in JavaScript when inputs might be objects. Objects can emulate strings or override prototype methods to evade sanitization checks and inject arbitrary HTML.
**Prevention:** Always explicitly coerce input to a string primitive (e.g., `String(input)`) *before* performing security-critical operations like `.replace()`. Replace falsy checks with strict nullish checks (`if (input === null || input === undefined)`) to preserve valid inputs like `0`.
