...
## 2024-05-25 - Object-based XSS Bypass
**Vulnerability:** The manual HTML escaping functions `escapeHtml` and `formatMessage` in `chatInterface.js` relied on falsy checks (`!unsafe`) and lacked strict string coercion. This allowed object-based inputs (e.g., objects with custom `replace` methods or arrays) to bypass the `.replace()` string sanitization, potentially leading to Cross-Site Scripting (XSS) or TypeErrors. Additionally, valid falsy string inputs like `"0"` could be incorrectly dropped.
**Learning:** When creating custom string sanitization logic, failing to explicitly coerce the input to a primitive string can open the door to prototype pollution or object injection bypasses, where the input mimics a string but behaves differently during `.replace()`.
**Prevention:** Always coerce untrusted inputs to a `String` (e.g., `String(input)`) before performing sanitization, and use strict null checks (`input === null || input === undefined`) rather than general falsy checks to ensure legitimate false values like `"0"` are preserved.
