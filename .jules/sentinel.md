...
## 2025-09-02 - Object-based XSS Bypass in HTML Escaping
**Vulnerability:** The HTML escaping logic in `chatInterface.js` (`escapeHtml` and `formatMessage`) relied on implicit string coercion during regex `.replace()` and used a simple falsiness check (`!unsafe`), which could be bypassed by passing a custom object with a malicious `replace` function (Object-based XSS) and incorrectly dropped valid values like `0`.
**Learning:** Manual HTML escaping functions must be extremely strict about types. If an attacker can control the type of the input, they can exploit prototype pollution or type juggling to bypass the sanitization logic entirely.
**Prevention:** Always enforce strict coercion to primitives (e.g., `String()`) before executing string manipulation methods for sanitization, and use strict equality (`=== null || === undefined`) instead of falsiness to safely ignore empty states.
