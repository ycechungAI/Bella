...
## 2024-05-25 - XSS Bypass in Manual Escaping Functions
**Vulnerability:** The `escapeHtml` and `formatMessage` functions in `chatInterface.js` relied on truthiness checks (`if (!unsafe) return '';`) and lacked explicit string coercion before calling `.replace()`. This allowed an XSS bypass where an attacker could provide an object with a custom `replace` method (e.g., `{ replace: () => "<script>alert(1)</script>" }`) or suppress valid input like the number `0`.
**Learning:** Manual HTML escaping functions must enforce strict primitive coercion (like `String(input)`) before any string manipulation to prevent prototype pollution or object-based bypasses. Furthermore, strict equality checks (`=== null || === undefined`) must be used instead of falsiness to avoid breaking valid data types.
**Prevention:** Always cast inputs to strings explicitly (e.g., `String(val)`) when writing manual sanitization logic, and avoid relying on truthiness for parameter validation when numbers or booleans might be passed.
