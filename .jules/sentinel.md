...
## 2024-06-17 - XSS bypass via Object string coercion
**Vulnerability:** The HTML escaping functions (`escapeHtml` and `formatMessage`) in `chatInterface.js` performed string replacement without first enforcing string coercion. They also used falsy checks (`!unsafe`) which suppressed valid values like `0`. This could allow an XSS bypass if an object overriding `replace` or `toString` is provided.
**Learning:** Always explicitly coerce variables to strings (e.g., `String(unsafe)`) before calling string manipulation methods, especially when sanitizing input. Furthermore, strict equality checks for null/undefined (e.g., `if (unsafe === null || unsafe === undefined)`) should be used over boolean falsiness to prevent silently dropping valid zero values.
**Prevention:** Implement strict string coercion and strict null checking in input sanitization routines.
