...
## 2026-06-22 - XSS and TypeError via Un-coerced Input Objects
**Vulnerability:** In `chatInterface.js`, the manual HTML escaping functions (`escapeHtml` and `formatMessage`) relied on duck-typing truthiness (`if (!unsafe) return '';`) and immediately called `.replace()` on the input. This allowed an attacker to supply a custom object mimicking a string (e.g. `{ toString: () => '<img src=x onerror=alert(1)>' }`) to bypass validation or cause a Denial of Service by throwing a TypeError on non-string inputs (like integers or booleans). It also suppressed valid inputs like `0`.
**Learning:** Relying on implicit string coercion or falsiness checks in validation functions is brittle. A non-string primitive or an object can either crash the function or bypass string-specific checks.
**Prevention:** Always explicitly coerce inputs to strings (`String(input)`) before applying string operations like regex replacements. Ensure you handle `null` and `undefined` with strict equality to avoid suppressing valid falsy values like `0`.
