...
## 2026-06-27 - Object-based XSS Bypass
**Vulnerability:** XSS vulnerability in `chatInterface.js`'s `escapeHtml` and `formatMessage` functions due to object-based bypasses. By passing an object with a custom `replace` method or prototype pollution, an attacker could bypass HTML escaping because the string `.replace()` methods were implicitly trusted. Additionally, falsiness checks (`!unsafe`) suppressed valid falsy string values like "0".
**Learning:** Checking for falsiness and chaining `.replace` on an argument without strict primitive coercion (`String()`) allows for object-based bypasses. When building custom sanitization or escaping functions, you must strictly coerce inputs to primitive strings to prevent objects mimicking string behavior from bypassing checks.
**Prevention:** Always coerce untrusted input to a string using `String()` before applying string operations like `.replace()`. Also, use strict equality checks (`!== null && !== undefined`) rather than truthy/falsy checks to handle valid values like "0" properly.
