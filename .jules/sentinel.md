...
**Vulnerability:** The `download_models.js` script used `child_process.exec` to run `git clone` commands, constructing the command string via interpolation. This exposes the application to command injection if the arguments (`modelUrl` or `targetDir`) were ever derived from untrusted input.
**Learning:** Using `exec` executes commands within a shell, which interprets shell metacharacters. Even if inputs are currently hardcoded, this is a dangerous pattern in utility scripts that might be repurposed or expanded to take external arguments in the future.
**Prevention:** Always use `child_process.execFile` (or `spawn`) when executing system commands. `execFile` executes the given executable directly and passes arguments as an array, completely bypassing the shell and eliminating the risk of command injection via shell metacharacters.
