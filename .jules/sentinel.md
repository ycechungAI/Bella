...
## 2026-06-25 - XSS Bypass via Object Methods and Falsy Checks Data Loss
**Vulnerability:** The HTML escaping functions (`escapeHtml`, `formatMessage`) were vulnerable to an XSS bypass. They allowed passing an object with a custom `replace` method and a malicious `toString` method. Because the input wasn't coerced to a string before calling `.replace()`, the custom object could bypass sanitization entirely. Additionally, a check for `if (!unsafe)` caused valid falsy inputs (like the number `0`) to be returned as an empty string (data loss).
**Learning:** Checking for truthiness (e.g., `!input`) before sanitizing can lead to bugs where valid values like `0` or `false` are swallowed. Furthermore, when implementing manual string manipulation for security purposes (like `.replace()`), you must guarantee the input is a primitive string to avoid prototype pollution or custom object bypasses.
**Prevention:** Always use strict equality checks for `null` and `undefined` instead of falsiness. Enforce strict coercion to primitives (e.g., `String(input)`) before performing security-critical string operations.
