...
**Vulnerability:** Custom HTML escaping functions (`escapeHtml`, `formatMessage`) did not explicitly cast the input to a primitive string. They relied on object duck-typing for `.replace()`, allowing XSS bypasses if an attacker supplied an object with a malicious `.replace` prototype. Furthermore, relying on falsiness to exit early suppressed valid inputs like `0`.
**Learning:** When implementing manual HTML escaping in JavaScript, always explicitly coerce the input to a primitive string (e.g., `String(input)`) before performing regex replacements. This prevents prototype pollution or object-based bypasses. In addition, always use strict equality checks for `null` or `undefined` rather than relying on falsy checks to avoid silently dropping valid values.
**Prevention:** Enforce string primitive coercion and strict `null`/`undefined` checking in all sanitization and escaping utilities across the codebase.
