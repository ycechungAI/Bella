...
## 2026-06-13 - XSS Bypass via Missing Type Coercion
**Vulnerability:** The `escapeHtml` and `formatMessage` functions in `chatInterface.js` only checked for truthiness (e.g., `if (!unsafe) return '';`) before using `.replace()` methods, failing to explicitly coerce the input to a primitive string. This could allow an XSS bypass via an object with a custom `.replace` method (prototype pollution/type confusion) and mistakenly suppress valid falsy string inputs like `'0'`.
**Learning:** When writing custom HTML escaping functions, checking for null/undefined is insufficient if the input type isn't strictly controlled. If an attacker can inject a non-string object (e.g., through parsed JSON or DOM manipulation), they can bypass string-specific sanitization if the code assumes it's a string. Furthermore, truthy checks (`!unsafe`) incorrectly catch valid zero values.
**Prevention:** Always enforce strict type coercion (e.g., `String(unsafe)`) before applying string manipulation methods for sanitization, and use strict null checks (`=== null || === undefined`) instead of truthy evaluations.
