...
## 2024-05-26 - XSS Bypass due to Missing String Coercion
**Vulnerability:** The `escapeHtml` and `formatMessage` functions in `chatInterface.js` used a generic truthiness check (`if (!unsafe)`) and failed to explicitly coerce the input to a string before calling `.replace()`.
**Learning:** Checking for `!unsafe` suppresses valid inputs such as `'0'`, which are falsy in JavaScript. More critically, without explicitly converting inputs to strings, passing non-string objects (like arrays or custom objects with a malicious `replace` method) could lead to TypeErrors or bypass the HTML escaping logic, resulting in Cross-Site Scripting (XSS).
**Prevention:** When creating manual HTML escaping functions, always strictly check for `null` or `undefined` (e.g., `if (unsafe == null)`), and explicitly coerce the variable to a string (`String(unsafe)`) before applying string replacement operations.
