...
## 2026-04-03 - Command Injection in download_models.js
**Vulnerability:** The `download_models.js` script constructs shell commands by directly concatenating variables (`exec("git clone ... " + targetDir)`). This pattern represents a Command Injection risk. If the source of `models` were ever changed to be user-provided or external, it would be trivially exploitable.
**Learning:** Using `child_process.exec` with concatenated strings is dangerous. Even if the variables are currently hardcoded, this pattern violates defense-in-depth principles and can become a vulnerability if the code is refactored.
**Prevention:** Always use `child_process.execFile` or `child_process.spawn` with an array of arguments to ensure that arguments are safely passed to the executable and cannot be interpreted as shell commands.
