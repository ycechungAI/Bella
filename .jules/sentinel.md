...
## 2024-05-25 - XSS Bypass via Object Prototype / Type Coercion
**Vulnerability:** The `escapeHtml` and `formatMessage` functions in `chatInterface.js` used a simple `!unsafe` check which suppressed valid falsy input (like `0`) from being rendered, and also implicitly called `.replace()`. This allowed an attacker to bypass HTML escaping entirely by passing an object with a custom `replace` method (e.g. `{ replace: () => "<script>alert(1)</script>" }`).
**Learning:** Checking for truthiness (`!unsafe`) is insufficient for validation when handling generic message contents. Additionally, if the input is not strictly a primitive string, relying on the input's own `.replace` method enables trivial Object-based bypasses.
**Prevention:** In custom sanitization functions, explicitly check for `null` or `undefined` instead of falsy values, and always coerce inputs to primitives (`String(unsafe)`) before calling string manipulation methods like `.replace()`.
