...
## 2024-05-25 - XSS Bypass via Falsy/Object Injection
**Vulnerability:** The HTML escaping mechanism (`escapeHtml` and `formatMessage` in `chatInterface.js`) failed to sanitize the number `0` (returning an empty string due to `!unsafe` falsy check) and did not strictly coerce inputs to primitive strings. This allowed object-based bypasses (e.g., an object with a malicious `.replace` function) and broke rendering for valid falsy text like `0`.
**Learning:** Checking for truthiness (`if (!unsafe)`) is insufficient for string processing inputs, as it incorrectly handles the number `0` or string `'0'` depending on context. Furthermore, assuming inputs to `.replace()` are safe strings without explicit coercion allows attackers to prototype-pollute or inject objects with malicious `replace` or `toString` implementations, bypassing sanitization logic.
**Prevention:** Always explicitly check for `null` or `undefined` instead of falsy values when sanitizing text. Strictly coerce the input to a primitive string using `String(input)` before applying any `.replace()` or string formatting methods to guarantee type safety and prevent object-based XSS bypasses.
