...
## 2024-05-25 - Object-Based XSS Bypass via Custom replace Methods
**Vulnerability:** XSS vulnerability in `chatInterface.js`'s `formatMessage` and `escapeHtml` functions where user input was inserted into `innerHTML` after a manual sanitization step that could be bypassed. Attackers could supply an object with a custom `replace` or `toString` method, causing the string replacement logic to output an unescaped XSS payload. Furthermore, falsy values like `0` were being suppressed due to `!unsafe` checks.
**Learning:** Manual HTML escaping functions that call `.replace()` directly on inputs without strict string coercion are vulnerable to object-based bypasses in environments where objects can be passed. If an object is supplied with a malicious `replace` method, the string-based regex replacements fail to sanitize it, leading to arbitrary HTML/JS execution. Also, using `!unsafe` for checking empty values is a bad practice as it suppresses legitimate falsy inputs like `0`.
**Prevention:** Enforce strict string coercion (e.g., `String(value)`) before applying any string processing or sanitization logic to ensure the value is treated solely as a primitive string. Replace falsy checks (`!value`) with explicit equality checks (`value === null || value === undefined`) to safely handle `0` or `false`.
