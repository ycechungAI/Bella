...
## 2025-05-29 - Object-Based XSS Bypass in HTML Escaping
**Vulnerability:** The HTML escaping functions (`escapeHtml` and `formatMessage`) in `chatInterface.js` failed to coerce input to primitive strings before processing. This allowed object-based XSS bypasses (e.g., using custom `replace` methods on an object) and accidentally suppressed valid falsy values like '0' due to `if (!unsafe)` checks.
**Learning:** Checking for falsy values like `!unsafe` skips valid strings like `'0'`. Moreover, blindly calling `.replace()` on an input without enforcing string type `String(unsafe)` leaves the function vulnerable to inputs that masquerade as strings but override expected methods to bypass sanitization.
**Prevention:** Always enforce primitive coercion (e.g., `String()`) before processing user inputs in sanitization/escaping functions. Use strict null checks (`if (unsafe == null)`) instead of loose truthiness checks to avoid suppressing valid user inputs.
