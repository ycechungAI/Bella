...
## 2026-04-22 - Conflicting Content Security Policy (CSP) Directives
**Vulnerability:** The `index.html` file contained two separate `<meta http-equiv="Content-Security-Policy">` tags. Because browsers enforce the intersection (the most restrictive combination) of multiple CSPs, one tag inadvertently blocked directives required by the other (e.g., `blob:` for workers and media).
**Learning:** Adding multiple CSP headers or meta tags does not merge their allowed sources; it enforces both independently. If one policy omits a necessary source (like `blob:` for `worker-src`), the browser will block it, leading to "security theater" where developers might disable CSP entirely to regain functionality.
**Prevention:** Always consolidate CSP rules into a single, comprehensive `<meta>` tag or HTTP header to ensure predictable, unified policy enforcement.
