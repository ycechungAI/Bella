...
## 2026-07-31 - XSS Bypass via Object-Based Type Coercion
**Vulnerability:** The `escapeHtml` and `formatMessage` functions in `chatInterface.js` relied on falsiness checks (`!unsafe`) and lacked explicit string coercion before applying `.replace()`. This allowed an attacker to pass an object with a custom `replace` method or a specific prototype to bypass the XSS sanitization. Additionally, checking for falsiness incorrectly suppressed valid falsey values like `0`.
**Learning:** Manual HTML escaping functions must enforce strict coercion to primitives (like `String()`) before any string processing. Failing to do so opens the door to object-based XSS bypasses and prototype pollution attacks. Furthermore, explicitly checking for `null` and `undefined` instead of simple falsiness preserves valid zero values.
**Prevention:** Always use strict equality (`=== null || === undefined`) for validation and enforce type coercion (e.g., `String(input)`) before applying `.replace()` or other string methods on user input.
## 2025-08-01 - Command Injection Vulnerability in Model Downloader
**Vulnerability:** The `download_models.js` script used `child_process.exec` to run `git clone`, constructing the shell command with variables (`git clone --depth 1 ${modelUrl} ${targetDir}`). This allowed for command injection if an attacker could control `modelUrl` or `targetDir` via malicious input.
**Learning:** `child_process.exec` passes the entire command string to a shell instance, making it susceptible to shell injection (e.g., using `&&` or `;` to append commands). This pattern is particularly dangerous when constructing commands dynamically.
**Prevention:** Replace `child_process.exec` with `child_process.spawn` and pass arguments as an array (`spawn('git', ['clone', '--depth', '1', modelUrl, targetDir])`). `spawn` circumvents the shell interpretation layer entirely, neutralizing injection attempts.
