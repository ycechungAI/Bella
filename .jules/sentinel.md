...
## 2024-05-27 - XSS Bypass and TypeErrors in Manual HTML Escaping
**Vulnerability:** The manual HTML escaping functions (`escapeHtml` and `formatMessage`) in `chatInterface.js` assumed string inputs and checked for validity using truthiness (`!unsafe`). This allowed malicious objects with custom `replace` methods to bypass the escaping logic, leading to XSS. It also caused `TypeError`s when numbers were passed and improperly suppressed valid falsy string inputs (like `"0"` or `0`).
**Learning:** When implementing manual string manipulation functions (like HTML escaping), duck typing and truthiness checks are insufficient and dangerous. An attacker can construct an object that mimics the expected interface (e.g., has a `replace` method) but behaves maliciously, completely bypassing the sanitization logic.
**Prevention:** Always explicitly coerce inputs to strings (e.g., `String(unsafe)`) before applying string manipulation methods like `.replace()`. Additionally, use strict equality checks (`=== null || === undefined`) rather than truthiness checks (`!unsafe`) to handle inputs gracefully without accidentally suppressing valid falsy values like `0`.
