...
## 2026-06-04 - Fix XSS bypass via object injection in formatMessage
**Vulnerability:** The `formatMessage` and `escapeHtml` functions in `chatInterface.js` failed to coerce inputs to strings prior to executing sanitization using `.replace()`.
**Learning:** This lack of explicit type coercion allows for potential Object injection. If an attacker managed to pass an object with a custom `.replace()` method (or `.toString()` method coupled with custom `.replace()`), they could bypass the escaping mechanisms. Furthermore, legitimate falsy values (like `0`) were mishandled, returning empty strings or crashing. Strict type checking is crucial for defensive string escaping.
**Prevention:** Always cast the input explicitly using `String()` after checking for `null`/`undefined` before performing string manipulations in custom HTML escaping functions.
