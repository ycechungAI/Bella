...
## 2026-05-23 - XSS Bypass via Uncoerced Inputs & Client-side DoS
**Vulnerability:** The `escapeHtml` and `formatMessage` functions in `chatInterface.js` failed to coerce inputs to strings before escaping, allowing potential XSS bypasses or TypeErrors. Additionally, `sendMessage` lacked a maximum length check on the server side, complementing the UI constraint.
**Learning:** Checking `if (!unsafe)` drops valid inputs like `"0"` or `0`. When inputs are not strictly coerced using `String(content)`, non-string inputs could bypass regex replacements designed to prevent XSS. Also, UI length limits (`maxlength` attribute) do not prevent client-side DoS if `sendMessage` is called directly without length validation.
**Prevention:** Always coerce input values strictly using `String(value)` before performing any sanitization or logic. Ensure the input constraint matches the backend logic (enforcing limits explicitly in script).
