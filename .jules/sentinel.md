...
## 2026-05-20 - Unhandled TypeError from Missing String Coercion
**Vulnerability:** Input fields flowing to `.replace()` functions in `escapeHtml` and `formatMessage` did not explicitly coerce inputs. If a non-string object (e.g. Number, Boolean, Object) flowed into these functions, it would throw an unhandled `TypeError`, potentially causing denial-of-service or logic bypasses in the UI.
**Learning:** Even when dealing with text-based UI layers, do not assume inputs are strings. Data-binding quirks, API response changes, or prototype pollution could result in non-string payloads hitting string methods, leading to fatal crashes rather than safe fallback.
**Prevention:** Always cast dynamically-typed inputs to expected primitive types (e.g., using `String(content)`) before calling string manipulation methods, especially in sanitization pipelines.
