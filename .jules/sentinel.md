...
## 2024-06-23 - Command Injection Risk in Shell Executions
**Vulnerability:** The `download_models.js` script used `child_process.exec` to run `git clone` with string interpolation. Although the current strings were safe, using `exec` to run shell commands poses a high risk of command injection if external inputs are ever incorporated.
**Learning:** `exec` spawns a shell and parses the command string, making it vulnerable to injection operators (like `;`, `&`, `|`). `spawn`, on the other hand, does not invoke a shell by default and passes arguments directly to the executable as an array, making it much safer.
**Prevention:** Always use `child_process.spawn` instead of `child_process.exec` when invoking external commands, and pass arguments as an array rather than interpolating them into a single command string.
