...
## 2025-07-28 - XSS Bypass in Manual HTML Escaping Functions
**Vulnerability:** The manual HTML escaping functions `escapeHtml` and `formatMessage` were vulnerable to XSS bypass via prototype pollution or object-based inputs (e.g., passing an object with a custom `replace` method or an object whose `toString` returns malicious HTML). Additionally, using a simple truthiness check (`if (!unsafe)`) incorrectly suppressed valid numeric inputs like `0`.
**Learning:** When writing custom HTML escaping routines, it's critical to strictly coerce the input to a primitive string (`String(unsafe)`) *before* applying any regular expression replacements. This guarantees that malicious object payloads cannot override the `replace` function or the `toString` conversion dynamically during processing. Furthermore, strict null checks (`if (unsafe === null || unsafe === undefined)`) must be used instead of simple truthiness to avoid data loss.
**Prevention:** Always use strict coercion (`String(...)`) and explicit null/undefined checks at the boundary of any manual HTML sanitization or escaping function, refusing to blindly trust the properties or prototype chain of the input.
