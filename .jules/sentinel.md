...
## 2024-06-14 - Object-based XSS Bypass in Manual HTML Escaping
**Vulnerability:** The custom `escapeHtml` and `formatMessage` functions in `chatInterface.js` relied on `String.prototype.replace` to sanitize input. Attackers could bypass XSS protections by providing an object with a custom `replace` method (e.g., prototype pollution) or bypass string-specific sanitization. Additionally, checking for falsy values (`!unsafe`) incorrectly suppressed valid inputs like the number `0`.
**Learning:** When writing manual HTML escaping or sanitization functions in JavaScript, it is critical to explicitly coerce inputs to strings (e.g., using `String(value)`) before applying string operations. Furthermore, type coercion issues should be avoided by strictly checking for `null` and `undefined` instead of generic truthiness to prevent suppressing valid falsy values like `'0'` or `0`.
**Prevention:** Always enforce primitive type coercion (e.g., `String()`) at the boundaries of manual escaping functions and use strict equality checks (`=== null || === undefined`) for validation.
