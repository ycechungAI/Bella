...
## 2025-09-08 - XSS bypass via Object Coercion in formatMessage/escapeHtml
**Vulnerability:** The HTML escaping functions (`escapeHtml`, `formatMessage`) relied on duck-typing falsiness (`!unsafe`) and direct string replacement (`unsafe.replace(...)`). If an attacker could inject a non-string object (e.g., an array or an object with a custom `replace` method or one missing it entirely), it could bypass the `!unsafe` check and either cause a Denial of Service (crash via `TypeError: .replace is not a function`) or potentially bypass sanitization if a custom `replace` method returned a malicious string.
**Learning:** Native string methods like `.replace` are not inherently type-safe when the input type is not guaranteed to be a primitive string. Relying on implicit truthiness for sanitization boundaries is fragile.
**Prevention:** Always explicitly coerce inputs to strings (`String(input)`) *before* performing any string-based sanitization or validation, and use strict equality checks (`=== null || === undefined`) for empty states rather than truthiness to properly handle values like `0` or `false`.
