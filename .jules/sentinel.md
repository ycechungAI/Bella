...
## 2024-05-24 - Client-side DoS via Object Key and Method Shadowing
**Vulnerability:** External API calls incorrectly defaulted to a 10-second timeout despite explicitly passing `timeout: 60000` in the fetch options, leading to premature connection termination (client-side DoS) during slow LLM responses.
**Learning:** This bug was caused by two JavaScript shadowing issues. First, the `cloudAPI.js` class had a duplicated `fetchWithTimeout` method definition where the later version shadowed the first, ignoring the `options.timeout` extraction and forcing a hardcoded default of `10000`. Second, the options object literal passed to the API methods contained a duplicated key (`timeout: 60000` followed by `timeout: 10000` at the end), effectively overwriting the intended 60s timeout due to object property reassignment. Furthermore, error handling leaked internal timer states.
**Prevention:** Eliminate duplicate class method definitions. When constructing configuration objects, ensure keys are not duplicated, as the last defined key will silently overwrite the previous ones. Always catch timeout-related errors and throw a generic message rather than interpolating raw timer values to avoid internal state leakage.
