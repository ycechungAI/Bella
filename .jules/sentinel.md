...
## 2026-04-16 - Command Injection via `child_process.exec`
**Vulnerability:** The `download_models.js` script used `child_process.exec` to run `git clone` with string interpolation for URLs and directory paths. This is a severe command injection vulnerability if any of the inputs (like `modelUrl` or `targetDir`) could be externally influenced.
**Learning:** `child_process.exec` spawns a shell and parses the entire string command, which makes it inherently vulnerable to command injection. Even in internal scripts, it's a dangerous pattern to establish.
**Prevention:** Always use `child_process.execFile` or `child_process.spawn` when running external commands with variables. These functions pass arguments as an array directly to the executable, bypassing the shell and eliminating the command injection vector.
