...
## 2024-05-24 - Explicit String Coercion for HTML Escaping
**Vulnerability:** XSS bypass and TypeErrors in `chatInterface.js`'s `formatMessage` and `escapeHtml` functions due to missing type validation before running `.replace()` with regexes. Attackers could pass an object with a custom `toString` or array to bypass the replace logic or cause app crashes.
**Learning:** Implicit type coercion or assumption of string inputs when sanitizing user input is unsafe. If an attacker passes an array (e.g., `["<script>"]`) or an object without a standard string representation, `.replace()` might throw a TypeError (causing DoS) or fail to execute correctly. Explicitly converting the input to a string (`String(unsafe)`) ensures that whatever data structure is passed, it is converted into its string equivalent *before* the escaping logic runs, neutralizing both XSS and TypeError risks.
**Prevention:** Always explicitly coerce inputs to strings (e.g., `String(input)`) *before* applying string manipulation functions like `.replace()` for sanitization, and properly handle `null`/`undefined` to fail securely or return empty strings.
