...
## 2024-05-25 - XSS Bypass via Object Injection and Truthiness Checks
**Vulnerability:** The `escapeHtml` and `formatMessage` functions in `chatInterface.js` relied on truthiness checks (e.g., `if (!unsafe) return '';`) and lacked explicit coercion to primitives before calling `.replace()`. This allowed an attacker to bypass XSS protections by passing an object with a custom `replace` method (prototype pollution/object injection) and suppressed valid falsy values like `0`.
**Learning:** Truthiness checks are insufficient for input validation as they incorrectly handle valid inputs like `0` or `false`. Furthermore, calling string methods like `.replace()` directly on untrusted input without enforcing a primitive string type can lead to TypeErrors or allow malicious objects to hijack the execution flow.
**Prevention:** Always use strict equality checks for `null` and `undefined` when validating optional inputs. Critically, always coerce untrusted input to a primitive string (e.g., `String(input)`) before performing any string manipulation or HTML escaping to guarantee that the intended string methods are executed.
