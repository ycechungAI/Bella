...
## 2024-05-25 - XSS Bypass via Object Prototype / Type Coercion in Manual Escaping
**Vulnerability:** The manual HTML escaping functions (`escapeHtml` and `formatMessage`) in `chatInterface.js` checked for input existence using truthiness (`if (!unsafe)`) and called `.replace()` directly on the input. This allowed a suppression of valid values like `0` (which is falsy) and opened a theoretical XSS bypass if an object with a custom `replace` method could be passed as input.
**Learning:** When implementing manual string sanitization or escaping, always coerce the input to a string primitive (e.g., `String(input)`) before performing replacements. This mitigates risks from prototype pollution or unexpected object types. Additionally, checking for missing values should use strict equality for `null` and `undefined` instead of simple truthiness to avoid eating valid falsy values.
**Prevention:** Enforce strict type coercion (`String()`) and strict null checks (`=== null || === undefined`) in any utility functions performing string manipulation for security purposes.
