...
## 2024-06-15 - Object-Based XSS Bypass in Custom Formatting
**Vulnerability:** XSS bypass in `chatInterface.js`'s `escapeHtml` and `formatMessage` functions where passing an object with a custom `replace` method could bypass the string replacement and execute arbitrary scripts.
**Learning:** Checking for falsiness (e.g., `!unsafe`) suppresses valid values like `0`. Additionally, implicitly assuming the input is a string allows attackers to pass objects that mimic string methods (prototype pollution or custom properties) to bypass security controls.
**Prevention:** Always enforce strict primitive string coercion (e.g., `String(input)`) before applying any custom formatting, replacements, or DOM insertions to ensure the input is securely handled. Always use strict equality checks (`null` or `undefined`) to prevent dropping valid inputs.
