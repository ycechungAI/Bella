...
## 2025-07-26 - Object-Based XSS Bypass in Manual Sanitization
**Vulnerability:** The `formatMessage` and `escapeHtml` functions in `chatInterface.js` relied on duck-typing for string methods (calling `.replace()` directly on the input). This allowed an attacker to pass a custom object with a malicious `replace` method that returned an unescaped XSS payload (e.g., `<img src=x onerror=alert(1)>`), bypassing the sanitization completely. Additionally, the falsiness check `if (!unsafe)` incorrectly suppressed valid falsy values like `0`.
**Learning:** When implementing manual HTML escaping functions, strict coercion to primitives (e.g., `String()`) must be enforced before any string processing. This prevents prototype pollution or object-based logic bypasses.
**Prevention:** Always use strict equality checks for `null` and `undefined` instead of falsiness, and explicitly coerce inputs to strings (e.g., `String(input)`) before applying `.replace()` or other string sanitization methods.
