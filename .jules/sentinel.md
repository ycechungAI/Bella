...
## 2026-05-18 - XSS Bypass and Crash via Non-String Input in Custom Sanitizer
**Vulnerability:** The custom `escapeHtml` and `formatMessage` functions in `chatInterface.js` assumed string inputs and called `.replace()` directly. If an attacker managed to pass an array or object (which some frameworks or API endpoints might output unexpectedly), the code would either throw a `TypeError` (causing a client-side crash/DoS) or, if coerced implicitly without escaping, bypass XSS protections.
**Learning:** Manual HTML sanitizers must strictly enforce type casting. JavaScript's dynamic typing means inputs from external sources (like APIs, workers, or dynamic UI events) cannot be guaranteed to be strings. Failing to cast leads to brittleness and security bypasses.
**Prevention:** Always explicitly coerce variables using `String(value)` before performing any string manipulation or escaping operations in security-critical paths, especially when avoiding external sanitization libraries like DOMPurify.
