...
## 2026-07-06 - XSS Filter Bypass via Type Confusion
**Vulnerability:** The `escapeHtml` and `formatMessage` functions in `chatInterface.js` expected string inputs but did not explicitly cast them, making them vulnerable to type confusion. If an attacker passed an object with a custom `.replace` method or took advantage of prototype pollution, the manual HTML escaping could be completely bypassed, leading to XSS. Additionally, numeric inputs caused TypeErrors.
**Learning:** When implementing custom string filtering or sanitization, it's critical to strictly cast inputs to a primitive string using `String()` before applying operations like `.replace()`. Furthermore, strict null/undefined checks should be used over generic truthiness to prevent suppressing valid inputs like the number `0`.
**Prevention:** Always enforce strong type coercion `String(content)` at the beginning of manual sanitization routines to eliminate object-based bypass attacks or unexpected TypeErrors.
