...
## 2026-05-07 - JavaScript-Level Input Length Validation
**Vulnerability:** The application relied solely on the HTML `maxlength="500"` attribute to limit user input length. This could be easily bypassed by attackers modifying the DOM or making API calls directly, potentially leading to client-side Denial of Service (DoS) due to excessive processing of massive strings.
**Learning:** HTML-level validation (like `maxlength` or `required`) is purely cosmetic and cannot be trusted for security. True defense-in-depth requires validation at the logic layer (in this case, the `sendMessage` function in `chatInterface.js`) before any state changes, DOM updates, or network requests occur.
**Prevention:** Always implement input validation in the JavaScript logic, in addition to any HTML attributes. Reject oversized inputs early in the execution flow with clear warnings, preventing them from consuming application resources.
