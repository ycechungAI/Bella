...
## 2024-05-26 - Input Validation Flaw in UI Message Processing
**Vulnerability:** The application was susceptible to client-side Denial of Service (DoS) attacks and performance degradation because it lacked logic-level length limits in `sendMessage()`. Although the HTML `<input>` had a `maxlength="500"` attribute, a malicious user could easily bypass this by pasting content or programmatically modifying the DOM, sending an excessively large payload to the rendering functions and backend APIs.
**Learning:** Relying solely on HTML attributes like `maxlength` is insufficient for enforcing input boundaries. It only provides UI guidance but does not restrict how data enters the application. Furthermore, a proper mitigation must reject the payload rather than silently truncating it, as truncating corrupts the user's intended prompt and sends an incomplete message to the AI, causing unexpected model behavior.
**Prevention:** Always implement logic-level input validation inside JavaScript functions that handle user input before the data is processed or forwarded. Reject overly large payloads outright and notify the user to adjust their input.
