...
## 2026-07-22 - XSS Bypass via Missing Type Coercion
**Vulnerability:** XSS bypass in `chatInterface.js`'s `escapeHtml` and `formatMessage` functions due to a lack of explicit string coercion and overly permissive falsiness checks. Attackers could pass malicious objects with custom `toString()` or `replace()` properties to bypass the regex replacement completely.
**Learning:** Manual HTML escaping mechanisms that blindly call string methods (like `.replace()`) without enforcing `String()` coercion are vulnerable to bypasses if the input is an object or array. Additionally, checking `if (!unsafe)` suppresses valid values like the number `0`.
**Prevention:** Always enforce strict coercion to primitives (e.g., `String()`) before executing string sanitization logic. Use explicit checks against `null` and `undefined` instead of falsiness to ensure robust and predictable behavior for all input types.
