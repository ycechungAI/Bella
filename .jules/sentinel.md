...
## 2026-07-07 - XSS Bypass via Object-based Payloads and Falsy Value Suppression
**Vulnerability:** The HTML escaping logic in `chatInterface.js` (`escapeHtml` and `formatMessage`) was vulnerable to prototype pollution or object-based bypasses because it didn't enforce string coercion before calling `.replace()`. An attacker could theoretically pass an object with a custom `replace` method to bypass escaping. Furthermore, it used a generic falsiness check (`!unsafe`), which would incorrectly suppress valid content like the number `0`.
**Learning:** When writing custom HTML escaping functions, always strictly coerce input to primitives (e.g., `String()`) before executing string manipulation methods. This prevents object-based injection payloads. Additionally, avoid generic falsiness checks for return defaults; check specifically for `null` or `undefined` so that values like `0` or `false` are rendered properly.
**Prevention:** Implement explicit `null`/`undefined` checks and enforce `String()` coercion in all utility functions that process potentially untrusted text for DOM insertion.
