...
## 2024-05-25 - Command Injection in Model Download Script
**Vulnerability:** The `download_models.js` script used `child_process.exec` to run `git clone` commands. While the variables used (`modelUrl` and `targetDir`) were seemingly controlled, interpolating strings directly into `exec` introduces a critical command injection vulnerability if those variables ever become user-controlled or originate from an untrusted source. It can also cause unexpected failures if paths contain spaces or shell metacharacters.
**Learning:** `child_process.exec` executes a shell, meaning string interpolation can be exploited via shell metacharacters (`;`, `|`, `&&`, etc.). Even for internal scripts, this pattern is dangerous and brittle.
**Prevention:** Always use `child_process.execFile` (or `spawn` without `{ shell: true }`) when executing external commands. `execFile` does not spawn a shell by default; it passes arguments directly to the executable as an array, completely mitigating command injection risks and safely handling arguments with spaces.
