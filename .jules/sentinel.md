...
## 2026-07-04 - XSS via Object Prototype Bypass in HTML Escaping
**Vulnerability:** The `escapeHtml` and `formatMessage` functions in `chatInterface.js` were vulnerable to an object-based XSS bypass. They relied on implicit string coercion and falsy checks (`if (!unsafe)`). An attacker could pass a malicious object (e.g., `{ replace: () => '<script>alert(1)</script>' }`) to bypass the escaping logic, as the `replace` method of the object would be called instead of the string `replace` method. Additionally, valid inputs like the number `0` were incorrectly treated as falsy and resulted in an empty string being returned.
**Learning:** When writing manual HTML escaping functions, strict coercion to a primitive string (e.g., `String(unsafe)`) must be enforced before applying string operations. This prevents prototype pollution and object-based bypasses. Relying on falsy checks (`!unsafe`) can also suppress valid values like `0`. Strict equality checks for `null` and `undefined` are safer.
**Prevention:** Enforce string coercion using `String()` and use strict equality checks (`!== null && !== undefined`) for input validation in sanitization functions.
