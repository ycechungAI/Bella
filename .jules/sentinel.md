...
## 2024-05-25 - Command Injection Risk in script execution
**Vulnerability:** A Command Injection vulnerability existed in `download_models.js` where user-controlled or external data (`modelUrl`, `targetDir`) was concatenated directly into a string executed by `child_process.exec()`. If those variables contained shell metacharacters (e.g. `; rm -rf /`), they could execute arbitrary commands.
**Learning:** Using `child_process.exec()` with string interpolation is inherently dangerous as it runs the string inside a shell, allowing shell injection attacks.
**Prevention:** Always use `child_process.execFile()` or `child_process.spawn()` with an array of arguments when executing external commands. This passes arguments directly to the executable, bypassing the shell and eliminating command injection risks.
