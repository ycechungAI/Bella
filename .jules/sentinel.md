...
## 2024-05-25 - XSS Bypass via Missing String Coercion in HTML Escaping
**Vulnerability:** The custom HTML escaping functions (`escapeHtml` and `formatMessage`) in `chatInterface.js` failed to strictly coerce inputs to strings and used a truthiness check (`if (!unsafe) return '';`). This allowed XSS bypasses using objects with custom `replace` methods (e.g., `{ toString: () => '<script>alert(1)</script>', replace: () => '<script>alert(1)</script>' }`) and suppressed valid user inputs like `'0'` or `0`.
**Learning:** When implementing manual string sanitization or escaping functions, you cannot assume the input is a string primitive. Attackers can pass objects that duck-type as strings to bypass replacement logic if string coercion isn't explicitly enforced. Additionally, relying on truthiness to check for empty inputs inadvertently filters out valid falsy primitives like the number `0` or string `'0'`.
**Prevention:** Always explicitly coerce inputs to strings (e.g., using `String()`) before executing string manipulation methods like `.replace()`. Check for missing inputs using strict equality against `null` and `undefined` instead of generic truthiness.
