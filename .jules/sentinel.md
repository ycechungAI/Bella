...
## 2024-05-25 - XSS Bypass via Object properties
**Vulnerability:** The HTML escaping function `escapeHtml` used a truthy check (`if (!unsafe)`) and string replacement (`unsafe.replace(...)`) without strict primitive string coercion. An attacker could bypass XSS protections by passing a custom object with a `replace` method, executing malicious functions instead of string replacements. It also suppressed `0` falsiness values.
**Learning:** Checking truthiness drops valid falsy values like `0`. More critically, if string-manipulation methods are called on unverified inputs, prototype pollution or crafted objects with their own implementations of properties like `replace` can lead to bypassing escaping routines completely.
**Prevention:** Always ensure HTML escaping routines do an explicit `String(input)` coercion before calling `.replace()` or other string methods. Use strict equality checks (`input === null || input === undefined`) rather than simple falsiness (`!input`) to avoid suppressing valid outputs.
