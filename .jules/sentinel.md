...
## 2025-10-24 - Object-Based XSS Bypass in Manual HTML Escaping
**Vulnerability:** The manual HTML escaping functions (`escapeHtml` and `formatMessage`) in `chatInterface.js` failed to coerce their input to a primitive string before calling `.replace()`. If an attacker managed to inject an object mimicking a string (e.g., `{ replace: () => "<script>alert(1)</script>" }`), the escaping logic was entirely bypassed, leading to Cross-Site Scripting (XSS). Additionally, using `if (!unsafe)` incorrectly suppressed valid falsy values like the number `0`.
**Learning:** When writing custom input sanitization or formatting functions, one cannot assume the input will always be a primitive string. JavaScript's dynamic typing allows object injection where strings are expected. Furthermore, falsy checks (`!value`) are often too broad and can cause functional bugs by eating legitimate values like zero.
**Prevention:** Always enforce strict type coercion (e.g., using `String(unsafe)`) before performing string-specific operations like `.replace()`. Use strict equality checks for nullish values (`if (unsafe == null)`) instead of truthiness checks to properly handle valid falsy primitives like `0`.
