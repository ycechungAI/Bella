...
## 2024-05-26 - XSS Bypass via Object Coercion in HTML Escaping
**Vulnerability:** The manual HTML escaping functions (`escapeHtml` and `formatMessage`) in `chatInterface.js` did not strictly coerce input to strings. This allowed potential XSS bypasses using objects with custom `replace` or `toString` methods (e.g. prototype pollution or object-based bypasses), and erroneously suppressed valid falsy values like `0`.
**Learning:** Checking for falsiness (`!content`) is insufficient and can suppress valid inputs. Furthermore, calling string methods directly on untrusted input can be exploited if the input is not a primitive string but an object that mimics string behavior to bypass sanitization.
**Prevention:** Always enforce strict coercion to primitives (e.g., `String(input)`) before string processing, and use strict equality checks (`!== null && !== undefined`) when validating inputs for manual sanitization functions.
