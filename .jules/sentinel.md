...
## 2024-05-25 - XSS Bypass via Missing String Coercion
**Vulnerability:** The custom `escapeHtml` and `formatMessage` methods in `chatInterface.js` checked input using `!unsafe`, which incorrectly suppressed valid falsy values (like `0`). More critically, they did not coerce the input to a primitive string before calling `.replace()`. An attacker could pass an object with a custom `replace` method or rely on prototype pollution to bypass HTML escaping entirely.
**Learning:** When implementing manual string sanitization or formatting logic in JavaScript, checking truthiness is insufficient and error-prone. Without explicitly casting to `String()`, object-based inputs can hijack string methods, circumventing XSS protections.
**Prevention:** Always enforce strict type coercion (e.g., `String(input)`) before performing string manipulations for security purposes. Use explicit checks against `null` and `undefined` instead of falsiness to ensure values like `0` or `false` are properly stringified and escaped.
