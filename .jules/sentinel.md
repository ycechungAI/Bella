...
## 2026-04-20 - Command Injection in Model Downloader
**Vulnerability:** The script `download_models.js` used `child_process.exec` to run git clone commands, concatenating variables directly into the command string. This is a critical command injection risk, as it runs in a full shell environment.
**Learning:** `child_process.exec` spawns a shell and parses the entire string, which is dangerous if any part of the string is derived from external or untrusted sources (even if hardcoded initially, it's a poor pattern that is easily exploitable if refactored). `child_process.execFile` is safer because it executes the specified program directly without spawning a shell by default, taking arguments as an array.
**Prevention:** Always use `execFile` or `spawn` with arguments passed as an array rather than concatenating strings for `exec`, mitigating shell injection risks entirely.
