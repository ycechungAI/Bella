...
## 2026-05-28 - XSS Bypass and TypeError via Non-String Inputs in HTML Escaping
**Vulnerability:** The `formatMessage` and `escapeHtml` functions in `chatInterface.js` assumed inputs were always strings. This allowed XSS bypasses if an attacker provided an object with a malicious `replace` method, caused TypeErrors when rendering numbers, and accidentally suppressed valid falsy values like '0' due to `!unsafe` checks.
**Learning:** Manual HTML escaping functions must strictly coerce inputs to primitives (e.g., `String()`) before processing. Relying on Duck Typing for string methods (`.replace()`) in security-critical sanitization opens the door to prototype pollution or object-based bypasses.
**Prevention:** Always explicitly check for `null`/`undefined` and coerce inputs to strings using `String(value)` before applying regex replacements for HTML escaping.
