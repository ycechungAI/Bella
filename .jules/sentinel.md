...
## 2025-05-27 - XSS Bypass via Non-String Inputs
**Vulnerability:** XSS bypass in `chatInterface.js`'s `formatMessage` and `escapeHtml` functions. The logic blindly called string `.replace()` on the input. This allowed non-string inputs (like objects with a custom `toString()` returning malicious HTML) to bypass validation. Furthermore, simple truthiness checks (e.g. `if (!unsafe) return ''`) suppressed legitimate falsy inputs like '0'.
**Learning:** Security functions that expect strings must explicitly cast input to a string. An attacker could otherwise supply an object or numeric type that JavaScript's dynamic typing might implicitly convert to a string *after* security checks or during concatenation (e.g., inside `innerHTML`). Additionally, relying on `!value` for null/undefined checks is unsafe as it discards valid data like `"0"` or `0`.
**Prevention:** Always coerce input using `String(value)` before performing regex replacements for sanitization. Use explicit `=== null || === undefined` checks instead of loose falsiness checks.
