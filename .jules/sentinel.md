...
## 2024-05-25 - Object-based XSS Bypass via Custom `.replace`
**Vulnerability:** XSS vulnerability in `chatInterface.js`'s `escapeHtml` and `formatMessage` functions where user input was not coerced to a string before calling `.replace()`. An attacker could pass an object with a custom `replace` method (e.g., `{ replace: () => 'malicious_payload' }`) to bypass the HTML escaping logic.
**Learning:** When writing custom HTML escaping functions, checking for falsy values (`!unsafe`) is insufficient and can lead to bugs (e.g., suppressing `'0'`). More importantly, implicitly trusting the `.replace` method on user-provided input allows object-based bypasses or prototype pollution attacks.
**Prevention:** Always strictly check for `null` or `undefined` (instead of falsiness) and explicitly coerce input to a string (e.g., `String(unsafe)`) before performing any string manipulation or escaping.
