...
## 2025-10-27 - XSS via String Coercion Bypass
**Vulnerability:** The HTML escaping functions (`escapeHtml` and `formatMessage`) did not explicitly coerce inputs to strings before calling `.replace()`. This allowed XSS bypasses via object prototype pollution or objects with a custom `replace` method (e.g., `{ replace: () => "<script>alert(1)</script>" }`). Furthermore, relying on falsiness checks (`if (!unsafe)`) suppressed valid values like `0`.
**Learning:** When creating manual sanitization functions, ensure strict data types. Bypasses are often found in how JavaScript handles implicit conversions and object methods.
**Prevention:** Always enforce strict coercion to a string primitive (e.g., `String(input)`) before applying regex replacements. Additionally, use explicit `null` and `undefined` checks (`input === null || input === undefined`) rather than simple falsiness to prevent unintended logic errors with valid falsy values like `0`.
