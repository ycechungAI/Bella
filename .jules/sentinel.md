...
## 2026-05-20 - Command Injection Risk in Model Downloader
**Vulnerability:** The `download_models.js` script used `child_process.exec` with string interpolation to execute `git clone` commands. This pattern is vulnerable to command injection if any of the interpolated variables (like URLs or paths) are manipulated or controlled by user input.
**Learning:** Using `exec` spawns a shell which interprets shell metacharacters, inherently introducing command injection risks. Even in utility scripts or configurations where inputs seem safe, this anti-pattern shouldn't be used as future refactoring could inadvertently expose it to malicious input.
**Prevention:** Always use `child_process.execFile` or `child_process.spawn` with an array of arguments. These methods pass arguments directly to the executable without shell interpretation, effectively preventing command injection vulnerabilities.
