...
## 2024-05-25 - XSS via Prototype Pollution / Object Coercion in Manual Sanitizers
**Vulnerability:** The custom manual sanitization functions `escapeHtml` and `formatMessage` did not explicitly coerce inputs to strings (using `String()`) before executing string replacement operations (e.g. `.replace(...)`). This allowed an attacker to bypass sanitization by passing an object with a custom `.replace` method or a crafted `toString` method. Additionally, it checked for falsy values like `!unsafe` which could mistakenly silence valid inputs like `'0'`.
**Learning:** When implementing manual HTML sanitization, always strictly coerce inputs to primitives (e.g., `String()`) *before* executing string-processing logic to prevent object-based bypasses. Relying on implicit string coercion during regex replacement can be manipulated if the input is not a primitive string. Furthermore, when determining if an input should be sanitized or returned as an empty string, explicit checks for `null` and `undefined` are preferred over generic truthiness to prevent suppressing zero values.
**Prevention:** Use explicit coercion `String(content)` before chaining `.replace()` calls and check for nullish values (`if (content === null || content === undefined) return '';`) instead of `!content` when sanitizing user input.
