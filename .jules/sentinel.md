...
## 2026-04-04 - Object-based XSS Bypass in Custom HTML Escaping
**Vulnerability:** A vulnerability in `chatInterface.js`'s custom `escapeHtml` and `formatMessage` functions allowed object-based XSS bypass. If a user provided an object with a custom `.replace()` method (e.g., prototype pollution or object injection), the manual sanitization logic could be completely bypassed, injecting arbitrary HTML. Additionally, falsy scalar inputs like `0` were incorrectly suppressed, which can cause UI bugs.
**Learning:** Manual HTML escaping functions that depend on the `.replace()` string method without explicitly coercing the input to a primitive string type are vulnerable to object-based bypasses. Furthermore, simple truthy checks (`if (!unsafe) return '';`) are flawed as they inadvertently discard valid input like the number `0`.
**Prevention:** Always enforce strict coercion to a primitive string (e.g., using `String()`) before executing string manipulation methods. Check for `null` and `undefined` explicitly rather than relying on loose truthiness evaluations. Use dedicated DOM APIs like `textContent` when possible instead of `innerHTML`.
