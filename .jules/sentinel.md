...
## 2024-05-25 - XSS Filter Bypass via Non-String Inputs
**Vulnerability:** The HTML escaping mechanisms (`escapeHtml` and `formatMessage`) in `chatInterface.js` expected string inputs and used `.replace` directly. If passed a non-string value (e.g. from modified or malformed user payloads), it could either throw a `TypeError` causing a localized denial-of-service, or bypass sanitization if handled improperly by the JS engine. Falsy values like `0` were incorrectly suppressed due to `!unsafe` checks.
**Learning:** Defensive programming for string manipulation functions requires strict type checking or explicit coercion (`String()`). Simple truthiness checks like `if (!unsafe)` are dangerous when valid numeric values like `0` might need rendering.
**Prevention:** Always coerce untrusted inputs explicitly to the expected type (`String(input)`) before applying security-critical sanitization methods like regex replacements, and handle nullish checks explicitly (`=== null || === undefined`).
